...
1. [TODO]试在一个适当的域中给出$\sqrt{1+z}+\sqrt{1-z}$的一个单值分支的精确定义，并证明它是解析的。

2. [TODO]同样为$\log\log z$定义一个单值分支，并证明它是解析的。

3. [TODO]设$f(z)$在域$\Omega$内解析，并满足条件$|f(z)^2-1|<1$。证明在整个$\Omega$上或者$\operatorname{Re}(z)>0$，或者$\operatorname{Re}(z)<0$。
...
- 若$a\bar{c}-\bar{a}c\neq0$，通过配方化简得圆的方程：  
   $$\left|w+\frac{\bar{a}d-c\bar{b}}{\bar{a}c-\bar{c}a}\right|=\left|\frac{ad-bc}{\bar{a}c-c\bar{a}}\right|$$
   - 草稿纸上画了一会
...
2. ​**交比表示**​  
   用$\lambda=(z_1,z_2,z_3,z_4)$表示四点的24种排列的交比。
...
3. ​**圆上四边形的性质**​  
   ...
   并给出几何解释。
   - $1=|\frac{z_1-z_2}{z_1-z_3}\frac{z_4-z_3}{z_4-z_2}|+|\frac{z_1-z_4}{z_1-z_3}\frac{z_2-z_3}{z_2-z_4}|$ 
...
$$=(\overline{z-a},\frac{R^2}{z_1-a},\frac{R^2}{z_2-a},\frac{R^2}{z_3-a})$$
- 这里整体用线性变换 $T=\frac{R^2}{z}$
$$=(\frac{R^2}{\overline{z-a}},z_1-a,z_2-a,z_3-a)$$
- 这里整体用线性变换 平移 $T=z+a$
$$=(\frac{R^2}{\overline{z-a}}+a,z_1,z_2,z_3)$$
这一方程表明$z$的对称点为$z^*=\frac{R^2}{\overline{z-a}}+a$，或者说$z$及$z^*$满足方程
$$(z^*-a)(\overline{z-a})=R^2$$
因此$z$及$z^*$至圆心的距离的乘积$|z^*-a|\cdot|z-a|$等于$R^2$。此外，比$(z^*-a)/(z-a)$为正，这说明$z$及$z^*$位于由$a$引出的同一条半直线上。这提供了一个关于$z$的对称点的简单作图法（见图3-2）。注意$a$的对称点是$\infty$。
- 也就是 $z,z^*,a$ 三点共线，距离乘积为R^2,是个嵌套相似三角形，
...
2. 试将虚轴、直线$x=y$、圆$|z|=1$反射到圆$|x-2|=1$上。
...
6. 复平面中 假定一个线性分式变换把一组同心圆变换为另一组同心圆，证明圆半径之比不变。
...
7. 求一个可以把$|z|=1$与$|z-1/4|=1/4$变换为同心圆的线性变换。半径之比怎样变化？
   - 6是不是也可以用保持对称性做
...
一个线性变换$S$把有向圆$C$变换为一个以三重点$Sz_1, Sz_2, Sz_3$定向的圆. 从交比的不变性可知: $C$的左边和右边将对应于`象圆`的左边和右边.
...
当我们把一个未经推广的复平面考虑为扩充平面的一部分时, 无穷远点是特殊的. 因此, 可以根据$\infty$应位于有向圆`右边` 这一要求来定义所有有限圆的绝对正定向, 
- 这样, 左边的点组成圆的`内部`
- 而右边的点则组成圆的`外部`.
- 总结：根据 Im(交比)的正负
  - 正 右
...
- $Im (\infty,1,i,-1)=Im \frac{\infty-i}{\infty+1}\frac{1+1}{1-i}=Im(1+i) > 0$
  - 逆时针是正向

### 3.3.4 练习

1. [TODO]如果$z_1, z_2, z_3, z_4$为同一圆上的点，证明$z_1, z_3, z_4$和$z_2, z_3, z_4$当且仅当$(z_1, z_2, z_3, z_4) > 0$时确定同一定向。

2. [TODO]证明圆的切线垂直于过切点的半径（这样，圆的切线可以定义为与圆只有一个公共点的直线）。

3. [TODO]证明圆$|z - a| = R$的内部由满足不等式$|z - a| < R$的全部点$z$组成。

4. [TODO]两个有向圆在它们一个交点上的交角定义为它们在该点的相同取向的切线之间的夹角。用分析的推理而不是用几何观察证明：两个交点处的交角是互为反向的。
...
[ytb steiner circles](https://www.youtube.com/watch?v=_niSCpSMcxs)
...
2. [TODO]设变换
$$Sz=\frac{az+b}{cz+d}$$
的系数用条件$ad-bc=1$规范化，证明$S$是椭圆变换，当且仅当$-2<a+d<2$；$S$是抛物型变换，如果$a+d=\pm2$；$S$是双曲变换，如果$a+d<-2$或$a+d>2$。

3. 试证明：对某个整数$n$满足条件$S^nz=z$的线性变换$S$必是椭圆变换。
- $S:p(w)=k_sp(z)$
- $S^n:p(w)=k_s^np(z)$
- $|k_s^n|=1$
- $|k_s|^n=1$
...
4. [TODO]如果$S$是双曲的hyperbolic或斜坡的loxodromie，证明当$n\to\infty$时$S^nz$收敛到一个不动点，对所有的$z$。除了$z$与另一个不动点重合外都如此。（极限情形的这个不动点是吸收的attractive，另一不动点是排斥的 repellent。当$n\to\infty$时会发生什么情况？在抛物型情形又如何？）

5. [TODO]试求表示黎曼球面旋转的所有线性变换。
...
7. [TODO]一族变换以明显的方式（我们不准备精确叙述）依赖于某一数量的实参数。在所有线性变换的族中，试问一共有多少个实参数？在椭圆、双曲、抛物型变换族中各有多少？有多少线性变换可以把一给定的圆$C$保持不变？
...
# 绘制原始平面（网格线 + 圆）
plt.subplot(121)
plt.title("Original Plane (z)")
plt.grid(True)
plt.plot(x, np.zeros_like(x), 'k', lw=0.5)  # x轴
plt.plot(np.zeros_like(y), y, 'k', lw=0.5)  # y轴

# 绘制网格线（水平蓝，垂直绿）
for i in range(len(x)):
    plt.plot(X[:, i], Y[:, i], 'b', alpha=0.3, lw=0.5)  # 垂直线
for i in range(len(y)):
    plt.plot(X[i, :], Y[i, :], 'g', alpha=0.3, lw=0.5)  # 水平线
...
for r, color in zip(radii, colors):
    ...
    plt.plot(circle_x, circle_y, color=color, lw=2, alpha=0.8, label=f'r={r}')
...
# 绘制变换后的平面（网格线 + 圆）
plt.subplot(122)
plt.title("Transformed Plane (w = z + 1/z)")
plt.grid(True)
plt.xlabel('Re(w)'), plt.ylabel('Im(w)')

# 绘制网格线（保持原色区分）
for i in range(len(x)):
    plt.plot(U[:, i], V[:, i], 'b', alpha=0.3, lw=0.5)  # 垂直线
for i in range(len(y)):
    plt.plot(U[i, :], V[i, :], 'g', alpha=0.3, lw=0.5)  # 水平线
...
for r, color in zip(radii, colors):
    ...
    w_circle = f(z_circle)
    plt.plot(w_circle.real, w_circle.imag, color=color, lw=2, alpha=0.8)
...
# 参数设置
r_min, r_max = 0.2, 5.0    # w的模长范围（避免r=0）
num_points = 500            # 每条曲线的点数
theta_values = np.linspace(0, 2*np.pi, 24, endpoint=False)  # 固定theta的采样
r_values = [0.2, 0.5, 1.0, 2.0, 3.0]  # 固定r的采样
...
for r in r_values:
    ...
    x_w = r * np.cos(theta)
    y_w = r * np.sin(theta)
    ax1.plot(x_w, y_w, color='blue', linewidth=0.6, alpha=0.7)
...
for theta in theta_values:
    ...
    x_w = r * np.cos(theta)
    y_w = r * np.sin(theta)
    ax1.plot(x_w, y_w, color='red', linewidth=0.6, alpha=0.7)

ax1.set_title('Original w-plane', fontsize=14)
ax1.set_xlabel('Re(w)', fontsize=12)
ax1.set_ylabel('Im(w)', fontsize=12)
...
for r in r_values:
    theta = np.linspace(0, 2*np.pi, num_points)
    x_z = (r + 1/r) * np.cos(theta)
    y_z = (r - 1/r) * np.sin(theta)
    ax2.plot(x_z, y_z, color='blue', linewidth=0.6, alpha=0.7)
...
for theta in theta_values:
    ...
    x_z = (r + 1/r) * np.cos(theta)
    y_z = (r - 1/r) * np.sin(theta)
    ax2.plot(x_z, y_z, color='red', linewidth=0.6, alpha=0.7)

ax2.set_title('Mapped z-plane (z = w + 1/w)', fontsize=14)
...
3. 将弧$|z|=1,y\geq0$的补区域映成单位圆的外部，使无穷远点互相对应。
   - 端点a=-1,b=1
...
5. [TODO] 试将双曲线$x^{2}-y^{2}=a^{2}$的右边分支的内部映成圆盘$|w|<1$，使焦点对应于$w=0$，顶点对应于$w=-1$。（林德勒夫）

6. [TODO] 试将双纽线$\left|z^{2}-a^{2}\right|=\rho^{2}(\rho>a)$的内部映成圆盘$|w|<1$，使对称性不变。（林德勒夫）

7. [TODO] 将椭圆$(x/a)^{2}+(y/b)^{2}=1$的外部映成圆盘$|w|<1$，使对称性不变。

8. [TODO] 试将$z$平面在双曲线$x^{2}-y^{2}=1$右半支左边的部分映成一个半平面。（林德勒夫）
...
![](https://i-blog.csdnimg.cn/blog_migrate/28c916d58ef2afd68c15595b9421029d.png)
...
[youtube sqrt z黎曼面可视化](https://www.youtube.com/watch?v=mIOvmCyT4D)
